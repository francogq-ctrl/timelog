 ...
 *
 * Usage:
 *   node scripts/populate-slack-ids.js          # dry run, prints the plan
 *   node scripts/populate-slack-ids.js --apply  # actually writes to the DB
 *
 * Matching strategy: case-insensitive substring match on `name` first, then on
 * `email` local part. If no unambiguous match, the user is skipped and printed
 * for manual review.
 */
...
// From SLACK-BOT-SETUP.md — Slack IDs paired with the name fragment to match.
const SLACK_MAPPING = [
...
async function main() {
  ...
  for (const m of SLACK_MAPPING) {
    ...
    if (candidates.length === 0) {
      ...
      console.log(
        `⚠️  Ambiguous match for "${m.match}" (${m.slackId}): ${candidates.map((c) => c.email).join(", ")} — skipping`,
      );
    } else {
